...
impl DocManager {
    ...
    pub async fn load_doc_from_disk(&self, doc_id: &str, rev: i64) -> Result<Document, Box<Error>> {
        ...
        // get latest snapshot
        let snapshot_path = format!("{}/{}/{}{}", self.docs_folder, doc_id, latest_snapshot, SNAP_NAME_ID);
        ...
        if current_revision > 0 && Path::new(&rev_file_path).exists() {
            ...
            if should_merge_updates {
                ...
                doc.ydoc.transact_mut().apply_update(Update::decode_v1(&merged_updates).unwrap());
            } else {
                for update in revs {
                    doc.ydoc.transact_mut().apply_update(Update::decode_v1(&update).unwrap());
                }
            }
        }
    ...
    pub async fn update_doc(&self, doc_id: &str, document: &mut Document, update: &[u8]) -> Result<(), Box<dyn std::error::Error>> {
        ...
        let mut doc_metadata = match self.load_doc_file(doc_id) {
            ...
            Err(_) => return Err(Box::new(Error::new(ErrorKind::Other, format!("Failed to load doc metadata for doc {}", doc_id))))
        };
    ...
    async fn save_snapshot(&self, doc_id: &str, snapshot_id: u64, doc: &Document) -> io::Result<()> {
        ...
        //let state_vector = doc.ydoc.transact().state_vector();
    ...
    async fn save_rev(&self, file_path: &str,rev_data: &[u8]) -> io::Result<()> {
        ...
        file.write_all(&(rev_data.len() as u32 + timestamp_bytes.len() as u32).to_le_bytes()).await?;
        file.write_all(&timestamp_bytes).await?;
